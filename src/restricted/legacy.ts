 ...
 * 0.0-1.0).
 */
const REPORTING_ID_PREFIX_TO_SAMPLING_RATE = new Map([['0', 0.1], ['1', 0.01]]);
...
function isCallSampled(options: ReportingOptions): boolean {
  return Math.random() <
      (options.samplingRate ??
       REPORTING_ID_PREFIX_TO_SAMPLING_RATE.get(options.reportingId[0]) ?? 0.0);
}

function maybeSendHeartbeat(options: ReportingOptions) {
  if (Math.random() <
      (options.heartbeatRate ??
       REPORTING_ID_PREFIX_TO_HEARTBEAT_RATE.get(options.reportingId[0]) ??
       0.0)) {
